...
const cl = (
  ...
  if (typeof input === "string") {
    nodeArray = Array.from(document.querySelectorAll(input));
  ...
  if (action && classname) {
    nodeArray.forEach((element) =>
      element && element.classList[action](classname)
    );
  }
...
const getItalianMonth = (monthNumber: number) => {
  const months = [
    "gennaio",
    "febbraio",
    "aprile",
...
export { cl, getItalianMonth, setAttributes };
